...
public class Inventario : MonoBehaviour
{
    ...
    private void Update()
    {
        if (Input.GetKeyDown(KeyCode.I))
        {
            ...
            //esta parte sirve para activar o desactivar la condicion anterior
            if (inventarioVisible)
            {
                inventarioPanel.SetActive((inventarioVisible));
                //Apertura
                inventarioPanel.transform.localScale = Vector3.zero;
                inventarioPanel.transform.DOScale(1f, 0.5f).SetEase(Ease.OutBack);
            }
            else
            {
                //Cierre
                inventarioPanel.transform.DOScale(0f, 0.3f).SetEase(Ease.InBack)
                    .OnComplete(() => inventarioPanel.SetActive(false));
            }
        }
    }
